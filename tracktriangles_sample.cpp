...
int main(int argc, char** argv) {
	...
	while (cnt < 17)
	{
		...
		string filename_in = "sample_input/" + to_string(cnt) + ".png";
		//camera.read(frame);
		//camera >> frame;
		frame = imread(filename_in);
		if (frame.empty()) {
		...
		string filename_out = "sample_output/" + to_string(cnt) + ".png";
		imwrite(filename_out, frame);
